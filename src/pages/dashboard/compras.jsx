...
export function Compras() {
  ...
  const handleSave = async () => {
    ...
    if (!selectedCompra.id_proveedor || !selectedCompra.fecha_compra || selectedCompra.detalleCompras.length === 0) {
        // Mostrar alerta debajo de los campos en rojo
    ...
    if (insumosSeleccionados.length !== insumosUnicos.size) {
        // Mostrar alerta debajo del campo de insumos en rojo
        document.getElementsByName("id_insumo")[0].classList.add("border-red-500");

        Swal.fire('Error', 'No se pueden seleccionar insumos duplicados.', 'error');
        return;
    }
  ...
  return (
    ...
    <DialogHeader className=" text-black p-4">Crear Compra</DialogHeader>
